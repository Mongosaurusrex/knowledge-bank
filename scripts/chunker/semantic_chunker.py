...
class SemanticChunker:
    ...
    def split_into_sentences(self, text: str) -> List[str]:
        ...
        """
        # Python's `re` does not support variable-width look-behind, so we split
        # on terminal punctuation and then merge abbreviation-induced fragments.
        if not text.strip():
            return []
        ...
        for sentence in sentences:
            ...
            if merged and merged[-1].rstrip().lower().endswith(abbreviations):
                merged[-1] = merged[-1].rstrip() + " " + current
            else:
